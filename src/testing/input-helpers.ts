...
export function changeInputValue(fixture: ComponentFixture<any>, selector: string, value?: any): void {
  ...
  const hasInputEvent = element.name === 'select' || ['radio', 'checkbox', 'file'].includes(element.attributes.type as string);

  element.triggerEventHandler(hasInputEvent ? 'change' : 'input', {
    target:
  ...
  element.triggerEventHandler('blur', { target: { value } });
}
...
export function getInputValue(fixture: ComponentFixture<any>, selector: string): any {
